...
class HeteroscedasticObservationModel(LinearObservationModel):
    def __init__(self, Dx: int, Dz: int, link_function: str = "exp"):
        ...
        if link_function == "exp":
            ...
            self.A_vec = identity_flat * jnp.sqrt(2.0)
    ...
    def from_linear_model(
        cls, lin_om: LinearObservationModel, link_function: str = "exp"
    ) -> "HeteroscedasticObservationModel":
        ...
        if link_function == "exp":
            ...
            model.A_vec = L_flat * jnp.sqrt(2.0)
        model.C = lin_om.C
        model.d = lin_om.d
        W = np.random.randn(model.Dx, model.Dz + 1)
        model.W = jnp.array(W)
        model.update_observation_density()
        return model
    ...
    def filtering(
        ...
        # )
        cur_filter_density = (
            self.observation_density.affine_variational_conditional_transformation(
                prediction_density, x_t
            )
        )
        return cur_filter_density
    ...
    def update_observation_density(self):
        ...
        self.observation_density = (
            heteroscedastic_conditional.HeteroscedasticConditional(
                M=jnp.array([C]),
                b=jnp.array([self.d]),
                A_vec=self.A_vec,
                W=W,
    ...
    def _update_euclid_params(self, X, smooth_dict):
        ...
        params = {"C": self.C, "d": self.d, "A_vec": self.A_vec, "W": self.W}
    ...
    def get_params(self) -> dict:
        return {"C": self.C, "d": self.d, "A_vec": self.A_vec, "W": self.W}
