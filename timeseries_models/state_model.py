...
class LinearStateModel(StateModel):
    def __init__(self, Dz: int, noise_z: float = 1.0):
        ...
        self.A, self.b = .9 * jnp.eye(self.Dz), jnp.zeros((self.Dz,))
...
class NNControlStateModel(LinearStateModel):
    ...
    def _setup_control_func(self, control_func: callable) -> hk.Module:
        if control_func == None:
            control_func = hk.transform(lambda x: hk.nets.MLP([10, self.Dz * (self.Dz + 1)], activation=jax.nn.tanh, w_init=jnp.zeros)(x))
    ...
    def update_hyperparameters(
        ...
        **kwargs
        ...
        :type u: jnp.ndarray
        """
        self._update_network_params(smooth_dict, two_step_smooth_dict, control_z, **kwargs)
    ...
    def _get_Qz_stats(self, two_step_smooth_dict: pdf.GaussianPDF, control_z: jnp.ndarray,):
        two_step_smoothing_density = pdf.GaussianPDF(**two_step_smooth_dict)
        T = two_step_smoothing_density.R
    ...
    def _update_network_params(
        ...
        **kwargs
        ...
        def objective(params, smooth_dict, two_step_smooth_dict, control_z) -> float:
            smoothing_density = pdf.GaussianPDF(**smooth_dict)
            two_step_smoothing_density = pdf.GaussianPDF(**two_step_smooth_dict)
            self.state_density.control_func = lambda x: self.control_func_hk.apply(params, x)
            return -self.compute_Q_function(smoothing_density, two_step_smoothing_density, control_z)
            
        batch_objective = lambda params, smooth_dict, two_step_smooth_dict, control_z: jnp.mean(vmap(objective, in_axes=[None, {'Sigma': 0, 'mu': 0, 'Lambda': 0, 'ln_det_Sigma': 0}, {'Sigma': 0, 'mu': 0, 'Lambda': 0, 'ln_det_Sigma': 0}, 0])(params, smooth_dict, two_step_smooth_dict, control_z))
        params = self.net_params
        result  = minimize(batch_objective, params, 'L-BFGS-B', args=(smooth_dict, two_step_smooth_dict, control_z))
...
class LSEMStateModel(LinearStateModel):
    ...
    def _get_Qz_stats(self,
        ...
        two_step_smooth_dict: dict):
            ...
            sd_kk = sd_k.multiply(self.state_density.k_func, update_full=True)
            Ekk = jnp.sum(
                sd_kk.integral_light().reshape((T, self.Dk, self.Dk)), axis=0,
                )
    ...
    def _update_kernel_params(
        ...
        batch_objective = lambda params, smooth_dict, two_step_smooth_dict: jnp.mean(vmap(objective, in_axes=[None, {'Sigma': 0, 'mu': 0, 'Lambda': 0, 'ln_det_Sigma': 0}, {'Sigma': 0, 'mu': 0, 'Lambda': 0, 'ln_det_Sigma': 0}])(params, smooth_dict, two_step_smooth_dict))
        params = self.W
        result  = minimize(batch_objective, params, 'L-BFGS-B', args=(smooth_dict, two_step_smooth_dict))
        self.W = result.x
